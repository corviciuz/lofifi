...
pub fn progress_bar(state: &State, width: usize, colorize: bool) -> String {
    let mut duration = Duration::new(0, 0);

    let elapsed = if let Some(_) = state.current.track() {
        state.sink.get_pos()
    } else {
        Duration::new(0, 0)
    };
    ...
    let current_info = state.current.track();

    let palette_color = if let Some(current) = current_info {
...
pub fn action(state: &State, width: usize, colorize: bool) -> String {
    if let Some(info) = state.current.track() {
         let action = if state.sink.volume() < 0.01 {
             ActionBar::Muted
         ...
         action.format(state.bookmarked, width, colorize)
    } else {
         let progress = if let super::state::Current::Loading(p) = state.current { p } else { 0.0 };
         ActionBar::Loading(progress)
             .format(state.bookmarked, width, colorize)
    }
}
