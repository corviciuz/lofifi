...
impl DiscographyParser {
    ...
    pub async fn get_discography_with_tracks(
        ...
        if include_tracks {
            debug_log!("discography.rs - get_discography_with_tracks: fetching tracks for items");
    ...
    fn extract_title(link: scraper::ElementRef) -> String {
        ...
        let artist_override_sel = Selector::parse(".artist-override").unwrap();

        if let Some(el) = link.select(&title_sel).next() {
            let full = Self::normalize_text(&el.text().collect::<String>());

            if let Some(ao) = el.select(&artist_override_sel).next() {
                let artist_text_raw = Self::normalize_text(&ao.text().collect::<String>());

                let mut name = full.clone();
                let artist_text_clean = artist_text_raw
                    .trim()
                    .trim_start_matches(|c: char| c.is_whitespace())
                    .to_string();

                if !artist_text_raw.is_empty() { name = name.replace(&artist_text_raw, ""); }

                let artist_wo_by = Regex::new("(?i)^by\\s+")
                    .ok()
                    .and_then(|re| Some(re.replace(&artist_text_clean, "").to_string()))
                    .unwrap_or(artist_text_clean.clone());
                if !artist_wo_by.is_empty() {
                    for sep in [" ", " - ", " – ", " — "] {
                        let pattern = format!("{}{}", sep, artist_wo_by);
                        name = name.replace(&pattern, "");
                    }

                    if name.ends_with(&artist_wo_by) {
                        name = name[..name.len().saturating_sub(artist_wo_by.len())].to_string();
                    }
                }
    ...
    async fn fetch_html(client: &Client, url: &str) -> Result<String> {
        ...
        for attempt in 1..=5 {
            ...
            let mut req = client.get(url)
                ...
                .header("Accept-Language", "en-US,en;q=0.9,ru;ru-RU;q=0.8")
            ...
            if resp.status() == reqwest::StatusCode::TOO_MANY_REQUESTS {
                if attempt < 5 {
                    let delay = 20 * attempt as u64;
                    debug_log!("discography.rs - fetch_html: rate limited, retrying in {}s (attempt {}/5) for URL: {}", delay, attempt, url);
                    eprintln!("Rate limited: {} — retrying in {}s (attempt {}/5)", url, delay, attempt);
    ...
    async fn fetch_album_tracks(
        ...
        let all_items_data: Vec<(String, Option<u64>, String)> = items
            .iter()
            .filter(|item| item.item_type == "album" || item.item_type == "track")
            .map(|item| (item.url.clone(), item.id, item.item_type.clone()))
            .collect();

        if all_items_data.is_empty() {
            debug_log!("discography.rs - fetch_album_tracks: no albums or singles found to process");
            return;
        }

        debug_log!("discography.rs - fetch_album_tracks: found {} items, extracting...", all_items_data.len());
        println!("Found {} items, extracting...", all_items_data.len());

        let batch_size = 10;
        let mut processed = 0;

        for chunk in all_items_data.chunks(batch_size) {
            let handles: Vec<_> = chunk
                .iter()
                .map(|(url, _id, _item_type)| {
                    let url = url.clone();
                    let client = client.clone();
                    
                    tokio::spawn(async move {
                        match Self::get_album_tracks(&client, &url).await {
                            Ok(tracks) => (url.clone(), Ok((tracks, None::<String>))),
            ...
            for handle in handles {
                if let Ok((url, result)) = handle.await {
                    ...
                    match result {
                        Ok((tracks, _real_url)) => {
                            println!("  Item {}/{}: {} tracks", processed, all_items_data.len(), tracks.len());
                        ...
                        Err(e) => {
                            ...
                            println!("  {} Item {}/{}: {}", prefix, processed, all_items_data.len(), e);
                        }
                    }
                }
            }

            println!("Processed {}/{} items", processed, all_items_data.len());

            if processed < all_items_data.len() {
